         ...
         while running:
              ...
              for sock in reads:
                  if sock == self.server:
                     ...
                     parser_data =  f'client {_input.index(conn)-1} Joined!'
                     ...
                     data = receiver(sock)
                     if data:
                       msg = '\n[client ' +  str(_input.index(sock)-1) + ']' + data
                       for socks in writes:
                           sender(socks, msg)
                     else:
                       print(self.get_client_name(sock) + ' is hung up' )
...
class Client:
     ...
     def __init__(self, host, port):
         ...
         ddata = receiver(self.client).split('Joined!')
         print('CHAT ROOM SOCKET TCP USING SELECT')
...
def main():
    ...
    parser = argparse.ArgumentParser(description='socket with select')
    parser.add_argument('-t', help='target host', type=str)
    parser.add_argument('-p', help='target port', type=int)
    parser.add_argument('-name', help='name server or client', type=str)
    args = parser.parse_args()

    if args.name == 'server':
        a = ServerHandler(args.t, args.p)
        a.serving()
    else:
        a = Client(args.t, args.p)
        a.communicate()
